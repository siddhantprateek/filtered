...
read -p "Enter the target port to run Jenkins on: " TARGET_PORT

if ! [[ "$TARGET_PORT" =~ ^[0-9]+$ ]]; then
  echo "Invalid port number. Please provide a valid number."
  exit 1
fi

read -p "Enter the desired container name for Jenkins: " CONTAINER_NAME
...
docker run --name $CONTAINER_NAME --restart=on-failure --detach \
  ...
  --publish $TARGET_PORT:8080 --publish 50000:50000 \
  --volume $(pwd)/jenkins/jenkins-data:/var/jenkins_home \
...
echo "Jenkins is Ready on port $TARGET_PORT. Container name: $CONTAINER_NAME"
